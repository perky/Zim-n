...
scrlog.transparency = 100

scrlog.textxOffset = 2
scrlog.bghOffset = 2

scrlog.font = love.graphics.newFont(love.default_font)
...
function scrlog.draw()
	...
	love.graphics.rectangle(love.draw_fill, 0, 0, scrlog.width, scrlog.getHeight())
	love.graphics.setColor(255, 255, 255, bg_alpha)
	love.graphics.rectangle(love.draw_fill, scrlog.width-5, (scrlog.scroll/(scrlog.lineNum - scrlog.getShowLines())*(scrlog.getHeight()-2)), 5, 2)
	...
	for i = 1, scrlog.getShowLines() do
		if scrlog.lines[i + scrlog.scroll] then
			love.graphics.draw(scrlog.lines[i + scrlog.scroll], scrlog.textxOffset, (i*scrlog.fontHeight))
		end
	end
end
...
function scrlog.mousePressed(x, y, mouse)
	...
	if (mouse == love.mouse_left) then
		scrlog.toggle()
	
	elseif (mouse == love.mouse_wheelup) then
		scrlog.doScroll(-1)
	
	elseif (mouse == love.mouse_wheeldown) then
		scrlog.doScroll(1)
